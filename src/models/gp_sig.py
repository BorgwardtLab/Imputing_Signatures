...
class GP_Sig(nn.Module):
    def __init__(self, n_input_dims, out_dimension, n_mc_smps, n_devices, output_device, sig_depth=2):
        super(GP_Sig, self).__init__()
        likelihood = gpytorch.likelihoods.GaussianLikelihood().to(
                        output_device, non_blocking=True)
        clf = DeepSignatureModel(in_channels=n_input_dims, out_dimension=out_dimension, sig_depth=sig_depth)
