...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Background: Signature transform and Gaussian process adapters} % \section{Background}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
For an exposition on the properties of the signature transform and its use in machine learning, please refer to \citet{primer2016} or \citet[Appendix A]{kidger2019deep}.


% % Signatures of order $1$, i.e.\ terms of the form $\signature_{a,b}^i$ correspond to the \emph{increment} of a path. Terms of order $2$, i.e.\   $\signature_{a,b}^{i,j}$, for $i \neq j$, correspond to
% %     \emph{signed} areas above and below of a path.
% %     %
...
%                   clip               = false,
%                 ]%
%                   \addplot[no marks, smooth] table {data/Function.txt};
%                   \draw[cardinal] (-1.0, -1.0) -- node[anchor=west]
%                   {} (-1.0, 3.00); %{\scriptsize $\signature^1_{a,b}$}
%                 \end{axis}
%               \end{tikzpicture}
%     }%
%     \subcaptionbox{y}{   
...
%               clip               = false,
%             ]%
%               \addplot[no marks, name path=F, smooth] table {data/Function.txt};
%               \path[name path=A] (-1, 3) -- ( 3, 3);
%               \path[name path=B] (-1,-1) -- (-1, 3);
%               \addplot[pattern=north west lines, pattern color=cardinal] fill between[of=F and A];
%               \draw[cardinal] (-1, 3) -- ( 3, 3);
%               \draw[cardinal] (-1,-1) -- (-1, 3);
%               %\node at (1.75,1.5) {$\signature^{1,2}_{a,b}$};
%             \end{axis}
%           \end{tikzpicture}
%     }%
%     \subcaptionbox{z}{
...
%             ]%
%               \addplot[no marks, name path=F, smooth] table {data/Function.txt};
%               \path[name path=A] (-1,-1) -- ( 3,-1);
%               \path[name path=B] ( 3,-1) -- ( 3, 3);
%               \addplot[pattern=north west lines, pattern color=bleu] fill between[of=F and A];
%               \draw[bleu] (-1,-1) -- ( 3,-1);
%               \draw[bleu] ( 3,-1) -- ( 3, 3);
%               %\node at (0.5,0.5) {$\signature^{2,1}_{a,b}$};
%             \end{axis}
%           \end{tikzpicture}
%          }%
% \end{figure*}
...
%
where
% BR: does this have to be a separate equation? Same goes for the
% others, except for the mean calculation.
\begin{equation}
...
it first has to be converted in to a continuous path. 
%
...
pass the posterior mean, as in Equation~\eqref{eq:gp-mean}\footnote{%
  % BR: This is too much detail for something that should be clear;
...
% BR: Why not make this a stand-alone sentence? 'Notice that...
Even if MC sampling
...
We classify four real-world time series datasets, i.e.\
%
\begin{inparaenum}[(i)]
...
binary classification tasks, we optimize area under the precision-recall curve (as approximated via average precision) and also report AUROC. For multi-class
...
\begin{table}[tbp] %
    \caption{\texttt{CharacterTrajectories} dataset under label-based subsampling. The top three methods are highlighted: bold \& underlined, bold, underlined. All measures are reported as percentage points. Balanced accuracy (BAC) is the metric we optimized for. We further report accuracy and weighted AUROC (w-AUROC).}
    \centering
    %\small{
...
\begin{table}[tbp]
    \caption{\texttt{PenDigits} dataset under label-based subsampling. The top three methods are highlighted: bold \& underlined, bold, underlined. All measures are reported as percentage points. Balanced accuracy (BAC) is the metric we optimized for. We further report accuracy and weighted AUROC (w-AUROC)}
    \centering
    %\small{
...
whereas the panels indicate the subsampling strategy.
  %
  % BR: very stupid, but it just occurred to me that we have the 'R'
  % plot on the 'left', while the 'L' plot is on the 'right'. Can we
  % just flip that? A cursory reviewer might get confused here.
...
\begin{enumerate}
    \item linear interpolation: At a given imputation point, the previous and next observed data point are linearly interpolated. Missing values at the start or end of the time series are imputed with $0$ which for standardized data also corresponds to the mean.
