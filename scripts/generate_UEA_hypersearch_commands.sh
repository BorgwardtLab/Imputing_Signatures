...
for model_type in ${model_types[*]}; do
    for dataset in ${datasets[*]}; do
        ...
        for subsampler in ${subsamplers[*]}; do
                ...
                if [ "$preprocessing" == "true" ]; then
                    ...
                    if [ "$model" == "ImputedSignatureModel" ]; then
                        ...
                        for data_format in ${data_formats[*]}; do
                            ...
                            echo python $fit_module_path with $subsampler $model $dataset $data_format n_calls=1 n_random_starts=1 overrides.n_epochs=1 >> $outfile
                        done
                    fi
                else
                    if [ "$model_type" == "imputed" ]; then
                        ...
                        for data_format in ${data_formats[*]}; do
                            ...
                            outdir=experiments/${fit_module}/${dataset}/${subsampler}/${data_format}${model}
                            #write python command to outfile 
                            echo python $fit_module_path -F $outdir with $subsampler $model $dataset $data_format evaluation_metric=balanced_accuracy >> $outfile
                        done
                    else
                        #define output directory of current hypersearch experiment
                        outdir=experiments/${fit_module}/${dataset}/${subsampler}/${model}
                        if [ "$dataset" == "PenDigits" ]; then
                            echo Using custom subsampling for PenDigits..
                            #write python command to outfile
                            echo python $fit_module_path -F $outdir with ${subsampler}${dataset} $model $dataset evaluation_metric=balanced_accuracy >> $outfile
                        else 
                            #write python command to outfile
                            echo python $fit_module_path -F $outdir with $subsampler $model $dataset evaluation_metric=balanced_accuracy >> $outfile
                        fi
